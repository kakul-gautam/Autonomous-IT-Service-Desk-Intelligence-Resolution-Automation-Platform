...
def incidents_view(request):
    ...
    resolved_count = Ticket.objects.filter(resolution_status='Resolved').count()
    failed_count = Ticket.objects.filter(resolution_status='Failed').count()
    pending_count = Ticket.objects.filter(resolution_status='Pending').count()
